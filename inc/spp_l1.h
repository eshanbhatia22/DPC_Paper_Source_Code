...
enum FILTER_REQUEST {SPP_L1D_PREFETCH, SPP_LLC_PREFETCH, L1D_DEMAND, L1D_EVICT}; // Request type for prefetch filter
...
class SIGNATURE_TABLE {
  public:
    ...
    void read_and_update_sig_l1(uint64_t page, uint32_t page_offset, uint32_t &last_sig, uint32_t &curr_sig, int32_t &delta);
};
...
class PATTERN_TABLE {
  public:
    ...
    void update_pattern_l1(uint32_t last_sig, int curr_delta),
         //read_pattern_l1(uint32_t curr_sig, int *prefetch_delta, uint32_t *confidence_q, uint32_t &lookahead_way, uint32_t &lookahead_conf, uint32_t &pf_q_tail, uint32_t &depth);
         read_pattern_l1(uint32_t curr_sig, int *prefetch_delta, uint32_t *confidence_q, uint32_t &lookahead_way, uint32_t &lookahead_conf, uint32_t &pf_q_tail, uint32_t &depth, uint32_t pq_occupancy, uint32_t pq_SIZE, uint32_t mshr_occupancy, uint32_t mshr_SIZE);
};
...
class PREFETCH_FILTER {
  public:
    ...
    bool     check_l1(uint64_t pf_addr, FILTER_REQUEST filter_request);
};
...
class GLOBAL_REGISTER {
  public:
    ...
    void update_entry_l1(uint32_t pf_sig, uint32_t pf_confidence, uint32_t pf_offset, int pf_delta);
    uint32_t check_entry_l1(uint32_t page_offset);
};
