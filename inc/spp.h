...
class PATTERN_TABLE {
  public:
    ...
    void update_pattern(uint32_t last_sig, int curr_delta),
         ...
         read_pattern(uint32_t curr_sig, int *prefetch_delta, uint32_t *confidence_q, uint32_t &lookahead_way, uint32_t &lookahead_conf, uint32_t &pf_q_tail, uint32_t &depth, uint32_t pq_occupancy, uint32_t pq_SIZE, uint32_t mshr_occupancy, uint32_t mshr_SIZE);
};
...
class RUNAHEAD_TABLE {
  public:
	uint64_t page_num[RT_WAY];
	bool	 page_valid[RT_WAY];
	int		 lru[RT_WAY];

	uint64_t la_addr[RT_DEPTH][RT_WAY];
	uint32_t la_sig[RT_DEPTH][RT_WAY];
	uint32_t  la_conf[RT_DEPTH][RT_WAY];
	bool     la_valid[RT_DEPTH][RT_WAY];

	uint32_t ra_table_index;
	...
	RUNAHEAD_TABLE () {
		...
		for (int i=0; i < RT_WAY; i++) {
			page_valid[i] = 0;
			lru[i] = i;
			for (int j=0; j<RT_DEPTH; j++) {
				la_valid[j][i] = 0;
			}
		}
	}
	void	fast_forward(uint64_t &addr, uint32_t &sig, uint32_t &conf, uint32_t &depth);
	void 	fill_entry(uint64_t prefetch_addr, uint32_t sig, uint32_t conf);
